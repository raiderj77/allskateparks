#!/usr/bin/env python3
"""
Reseed allskateparks.com from OpenStreetMap Overpass API.

Pulls only properly-tagged skate parks (leisure=skateboard OR
sport=skateboard). Replaces src/data/locations.json — original was
bloated with non-skatepark entries like "Airsoft" (airsoft fields).

Source: 2026-04-19 directory rationalization audit. WS4 D2 ruling: re-seed.

OSM coverage for US skateparks is relatively good given the open-source
mapping community's enthusiasm for this amenity type. Expect 2000-4000
entries returned from the combined queries.
"""
import json
import re
import sys
...
(
  node["leisure"="skateboard"](area.us);
  way["leisure"="skateboard"](area.us);
  relation["leisure"="skateboard"](area.us);
  node["sport"="skateboard"](area.us);
  way["sport"="skateboard"](area.us);
  relation["sport"="skateboard"](area.us);
);
out center tags;
"""
...
def query_overpass():
    print("Querying OpenStreetMap Overpass API for US skateparks...")
    payload = urllib.parse.urlencode({"data": QUERY}).encode()
    req = urllib.request.Request(
        OVERPASS_URL,
        data=payload,
        headers={"User-Agent": "allskateparks-reseed/1.0 (raiderj77)"},
    )
...
def transform(osm_data):
    ...
    for el in elements:
        ...
        if lat is None or lng is None:
            skipped_no_coords += 1
            continue
        # Coordinate-based dedup (many OSM skateparks are double-tagged)
        ...
        if fee_tag in ("no", "") and access_tag not in ("private", "permissive"):
            amenities.append("Free access")
        ...
        location = {
            ...
            "lat": round(lat, 6),
            "lng": round(lng, 6),
            "description": f"{name} is a skate park in {descr_city}{descr_state}.",
            "amenities": amenities,
        }
...
def state_breakdown(locations):
    ...
    print(f"\nTotal: {len(locations)} skate parks across {len(counts)} state buckets")


def main():
    repo = Path("/home/rex/allskateparks")
    if not repo.is_dir():
